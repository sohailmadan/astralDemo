...
// Matches generateActivity.ts's abortSignal timeout (in seconds) — widened to 10 minutes after
// repeated real timeouts at 120s. Used only to derive an honest worst-case total below — never
// shown to the user directly as a per-attempt promise, since real per-attempt latency varies
// too much for that to read as anything but wrong.
const TYPICAL_ATTEMPT_SECONDS = 600;
